...
Full declared verification command run after all repairs: `CoreWcfServiceModelProjection\|InterfaceMemberEligibility` (Analysis.Tests) 9/9, `CoreWcf` (FrameworkModels.Tests) 20/20, `CoreWcfServiceOperation` (Scenarios.Tests) 5/5, `CoreWcfServiceOperation` (Wording.Tests) 1/1. Full-suite regression check: FrameworkModels.Tests 251/251, Scenarios.Tests 170/170, Analysis.Tests 197/207 (the 10 failures are the pre-existing `SD1102` multi-SDK MSBuildLocator registration conflict on unrelated fixtures — confirmed present in isolation, independent of this candidate, and unrelated to Issues #5/#7).
